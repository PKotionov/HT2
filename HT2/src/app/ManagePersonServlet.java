...
public class ManagePersonServlet extends HttpServlet {
    ...
    // сообщение о некорректном вводе имени, фамилии или отчества
    private final String INCORRECT_NAME_MESSAGE = "должно быть строкой от 1 до 150 символов из букв, цифр, знаков подчёркивания и знаков минус.<br />";
    ...
    private String validatePersonFMLName(Person person) {
        ...
        if (!person.validateFMLNamePart(person.getName(), false)) {
            error_message.append("Имя " + INCORRECT_NAME_MESSAGE);
        }

        if (!person.validateFMLNamePart(person.getSurname(), false)) {
            error_message.append("Фамилия " + INCORRECT_NAME_MESSAGE);
        }

        if (!person.validateFMLNamePart(person.getMiddlename(), true)) {
            error_message.append("Отчество " + INCORRECT_NAME_MESSAGE);
        }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // Действие (action) и идентификатор записи человека (id) над которой выполняется это действие.
